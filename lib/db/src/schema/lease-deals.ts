...
export const leaseDealsTable = pgTable("lease_deals", {
  ...
  state: text("state"),
  zipOrigin: text("zip_origin"),
  sourceType: text("source_type").notNull().default("manual"),
  status: text("status").notNull().default("published"),
  verifiedAt: timestamp("verified_at", { withTimezone: true }),
  effectiveThrough: timestamp("effective_through", { withTimezone: true }),
  moneyFactor: numeric("money_factor", { precision: 10, scale: 7 }),
  residualPct: numeric("residual_pct", { precision: 5, scale: 2 }),
